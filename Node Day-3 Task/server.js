...
mani.put("/student/:studentId/assignMentor/:mentorId", async (req, res) => {
  try {
    ...
    if (student.cMentor) {
      const Mentor = await Mentor.findById(student.cMentor);
      Mentor.students.splice(indexOf(student), 1);
      student.pMentor.push(student.cMentor);
    }
    if (student.pMentor.includes(nMentor._id)) {
      res.status(402).send("current Mentor and previous mentor cannot be same");
    } else {
